"""Statystyka ocen Gemini dla wyników ekstrakcji regex."""
...
DEFAULT_INPUT_PATH = PROJECT_ROOT / "test" / "dane_testowe_regex_llm.json"
...
def print_table(rows: list[dict[str, Any]]) -> None:
    ...
    print("Statystyka ocen Gemini dla ekstrakcji regex")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Statystyka pól rx_* z pliku dane_testowe_regex_llm.json."
    )
    parser.add_argument("--input", default=str(DEFAULT_INPUT_PATH), help="Plik wejściowy JSON.")
    parser.add_argument(
        "--fields",
        default="all",
        help="Pola do uwzględnienia: all albo lista po przecinku, np. typ,powiat_ocr.",
    )
    return parser
