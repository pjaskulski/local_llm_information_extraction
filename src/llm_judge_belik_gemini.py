"""LLM-as-a-Judge dla danych testowych SGKP wydobytych przez Bielika.

Skrypt weryfikuje pola wydobyte wcześniej przez lokalny model Bielik na podstawie tekstu hasła
i zapisuje wyniki oceny w polach z przedrostkiem ``bl_``.
"""
...
DEFAULT_INPUT_PATH = PROJECT_ROOT / "test" / "dane_testowe_bielik.json"
DEFAULT_OUTPUT_PATH = PROJECT_ROOT / "test" / "dane_testowe_bielik_llm.json"
...
BL_FIELDS = [f"bl_{field}" for field in TARGET_FIELDS]
DONE_FIELD = "_bl_judge_done"
DONE_FIELDS_FIELD = "_bl_judge_fields_done"
...
for field in TARGET_FIELDS:
    RESPONSE_SCHEMA["properties"][field] = {
        ...
        "description": (
            "correct, jeżeli Bielik poprawnie wydobył informację obecną w tekście; "
            "incorrect, jeżeli informacja jest błędna, nadmiarowa albo brakująca; "
            "not_applicable, jeżeli w tekście nie ma takiej informacji i Bielik "
...
Oceniasz wyłącznie zgodność danych Bielika z podanym tekstem hasła. Ignorujesz
wszystkie pola ludzkiej weryfikacji i nie sugerujesz się nimi.
""".strip()

JUDGE_PROMPT = """
Zweryfikuj pola danych wydobytych przez Bielika z hasła Słownika Geograficznego
Królestwa Polskiego.

Masz zwrócić JSON tylko z kluczami wymienionymi w sekcji POLA DO OCENY.

Dla każdego klucza zwróć jeden z trzech statusów:
- "correct": w tekście jest dana informacja i Bielik poprawnie ją wydobył.
- "incorrect": Bielik podał błędną informację, zmyślił informację, pomylił
  kategorię informacji albo pominął informację obecną w tekście.
- "not_applicable": w tekście nie ma danej informacji i Bielik również jej nie
...
4. Jeżeli Bielik podał część poprawnych elementów, ale pominął istotny element
   albo dodał element nieobecny w tekście, oceń całe pole jako "incorrect".
5. Pole powiat odpowiada informacji o powiecie w tekście.
...
12. W przypadku pola Typ poprawną wartością może być wyrażenie złożone z paru słów, jeżeli w tekście jest np. wieś rządowa,
    a Bielik rozpoznał "wieś" to nie jest w pełni poprawna wartość i należy ją oznaczyć jako "incorrect". Jeżeli w tekście jest wś i fol. a Bielik
...
Model Bielik miał wydobywać tylko informacje obecne w tekście hasła. Jeżeli tekst
nie zawierał danej informacji, pole miało zostać pominięte albo mieć wartość
null. Przy ocenie porównuj wynik Bielika z tekstem źródłowym i z poniższymi
definicjami pól.
...
- powiat: nazwa powiatu z tekstu, zwykle po "pow.".
...
def clean_record_for_judge(record: dict[str, Any]) -> dict[str, Any]:
    """Usuń pola oceny i zostaw tylko kontekst oraz dane Bielik."""
    cleaned = {
...
def record_is_done(record: dict[str, Any], fields: list[str]) -> bool:
    ...
    return all(field in done_set or f"bl_{field}" in record for field in fields)
            ...
            for future in done:
                ...
                try:
                    ...
                    for field, (value, explanation) in verdict.items():
                        bl_field = f"bl_{field}"
                        explanation_field = f"{bl_field}{EXPLANATION_SUFFIX}"
                        if value is None:
                            data[idx].pop(bl_field, None)
                            data[idx].pop(explanation_field, None)
                        else:
                            data[idx][bl_field] = value
                            if value is False:
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Weryfikacja dane_testowe_bielik.json przez Gemini jako LLM-as-a-Judge."
    )
    ...
    parser.add_argument(
        "--fields",
        default="all",
        help="Pola do oceny: all albo lista po przecinku, np. autor lub typ,powiat.",
    )
    ...
    parser.add_argument("--force", action="store_true", help="Przelicz także rekordy z istniejącymi polami bl_*.")
    parser.add_argument(
        "--no-resume",
        action="store_false",
        dest="resume",
